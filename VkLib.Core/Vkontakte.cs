...
namespace VkLib
{
    ...
    public class Vkontakte : IDisposable
    {
        ...
        /// <param name="usePlain">If true, return type is T, otherwise ApiResponse<T></T></param>
        /// <returns></returns>
        internal async Task<T> GetAsync<T>(string method, Dictionary<string, string> parameters,
            string baseUrl = MethodBase, bool usePlain = false)
        {
            ...
            // Get request Url.
            var urlString = GetUrl(string.Concat(baseUrl, method, "?"), parameters);
            ...
            switch (_jsonParsingType)
            {
                case JsonParsingType.UseStream:
                    ...
                    var apiStreamResponse = await DeserializeGetFromStream<ApiResponse<T>>(
                        urlString).ConfigureAwait(false);
                    ProcessErrors(apiStreamResponse.Error);
                ...
                case JsonParsingType.UseString:
                    ...
                    var apiStringResponse = await DeserializeGetFromString<ApiResponse<T>>(
                        urlString).ConfigureAwait(false);
                    ProcessErrors(apiStringResponse.Error);
        ...
        /// <returns></returns>
        internal static string GetUrl(string baseString, Dictionary<string, string> parameters)
        {
